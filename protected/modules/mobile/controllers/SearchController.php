...
class SearchController extends Controller {
    ...
    public function actionIndex() {
		...
		if(!is_null($model->search_type) && $model->search_type != "") {
			...
			$hotelLimit = $cityLimit = $stateLimit = 5;
			...
			//$this->render('intermediate',array('model' => $model, 'term' => $model->search_keyword, 'hotels'=>$hotelsArray, 'states'=>$statesArray, 'cities'=>$citiesArray));
...
// 			$totalCount = sizeof($hoteldata)+sizeof($citydata)+sizeof($statedata);
	...
	private function formatAllHotelsData($hoteldata, &$i, &$finaldata) {
		...
		foreach ($hoteldata as $htd){
			...
			//$finaldata[$i]['url']= Yii::app()->createUrl('mobile/hotel/detail',array('slug'=>$htd->slug, 'name'=>$htd->name));
			$finaldata[$i]['url']= Yii::app()->createUrl('mobile/hotel/detail',array('slug'=>$htd->slug));
			$i++;
		}
        return $finaldata;
	}
