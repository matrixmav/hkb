...
//This function corps image to create exact square images, no matter what its original size!
	...
	public static function slugCoursesUnique($model,$slug,$theme, $cId)
	{
		$id='';
		if($model=="Wiki")
		{
			if($theme<=3)
			{
			$Cat_ids=Categories::model()->findAll("parent_id in (select id from tbl_categories where parent_id=$theme)");
			foreach($Cat_ids as $c)
			$Category_ids[]=$c->id;	
			$theme1=implode(",",$Category_ids);
			}
			else 
			{
				$theme1=$theme;
	...
	if($model=="School")
		{
			if($theme<=3)
			{
			$Cat_ids=Categories::model()->findAll("parent_id=$theme");
			foreach($Cat_ids as $c)
			$Category_ids[]=$c->id;	
			$theme1=implode(",",$Category_ids);
			}
		...
		$slug = $this->makeSlugs($slug);
		if($model=="Wiki" || $model=="School") 
		{
			
			if(!empty($cId) && $cId!='')
				$flag=$model::model()->findAll("slug='$slug' AND category_id in ($theme1) AND id!=$cId");
			else
				$flag=$model::model()->findAll("slug='$slug' AND category_id in ($theme1)");
				foreach($flag as $flg)
				$id=$flg->id;
		...
		elseif($model=="Courses" )
		{
			...
			if(!empty($cId) && $cId!='')
				$flag=$model::model()->findAll("slug='$slug' AND category_id=$theme AND id!=$cId");
			else
				$flag=$model::model()->findAll("slug='$slug' AND category_id=$theme");
				foreach($flag as $flg)
				$id=$flg->id;
		}elseif($model=="Categories")
		{
			if(!empty($cId) && $cId!='')
				$flag=$model::model()->findAll("slug='$slug' AND parent_id =$theme AND id!=$cId");
			else
				$flag=$model::model()->findAll("slug='$slug' AND parent_id =$theme");
				foreach($flag as $flg)
				$id=$flg->id;
		...
		{
			if(!empty($cId) && $cId!='')
				$flag=$model::model()->findAll("slug='$slug' AND id!=$cId");
			else
				$flag=$model::model()->findAll("slug='$slug'");
				foreach($flag as $flg)
				$id=$flg->id;
		...
		}elseif($model=="Cuisine")
		{
			if(!empty($cId) && $cId!='')
				$flag=$model::model()->findAll("slugs='$slug' AND id!=$cId");
			else
				$flag=$model::model()->findAll("slugs='$slug'");
				foreach($flag as $flg)
				$id=$flg->id;
			
		}elseif($model=="Ambiance")
		{
			if(!empty($cId) && $cId!='')
				$flag=$model::model()->findAll("slugs='$slug' AND id!=$cId");
			else
		...
		}elseif($model=="Restaurant")
		{
			if(!empty($theme) && $theme!=''){
				$city = City::model()->find("id='$theme'");
				if($city){
					$cityName = $city->name;
					$CitySlug = $this->makeSlugs($cityName);
					$slug .= '-'.$CitySlug;
				}
			}
			
			if(!empty($cId) && $cId!='')
				$flag=$model::model()->findAll("url='$slug' AND id!=$cId");
			else
				$flag=$model::model()->findAll("url='$slug'");
				foreach($flag as $flg)
				$id=$flg->id;
	...
	public static function saveGroupForm($data)
	{
		$result['status']="Fail";
		$model=new GroupContactForm;
		if(!empty($data))
		{
			$model->attributes=$data;
			$date = DateTime::createFromFormat('d/m/Y', $model->reservation_time);
			$model->reservation_time = $date->format('Y-m-d h:i:s');
			$model->added_at=date("Y-m-d H:i:s");
			if($model->save()){
				$result['status']="Success";
				
				//$this->leadManager($model, $data['form_id']);	
				
				if(Yii::app()->language == "en"){
					$render = $this->renderPartial('//mail/admin_group_form_notification',array('model'=>$model), true);
					$emailConfig = array();
					$emailConfig['subject'] = Yii::app()->name.' : New Group Reservation';
					$emailConfig['body'] = $render;
					//$emailConfig['from'] = $model->email;
					$emailConfig['to'] =  array("arnaud.daniel@gmail.com","alexandre@itvillage.fr","krishna@itvillage.fr") ;
					if(!CommonHelper::sendMail($emailConfig)){
						//$result['status']="Fail";
					}
				}
    ...
    public static function getHotelStar($hotelId){
        $hotelOistObject = Hotel::model()->findByPk($hotelId);
        echo $starCount = $hotelOistObject->star_rating;
     ...
     * @return string
     */
    public static function getHotelStars($hotelId){
        ...
        if(!empty($starRatingCount)) {
            for($i=1;$i<=$starRatingCount;$i++ ){
                $stars[] = "<img src='/images/mobile/ratingstar_small.png'>";
            }
            return "<span style='display:inline-block;'>".implode($stars, "")."</span>";
        }
        return '';
    }
